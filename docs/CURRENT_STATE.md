...
| --- | --- | --- |
| SQLite health memory | Implemented and tested | Numbered, checksum-verified migrations; FK/integrity checks; immutable source artifacts; candidate revisions; append-only record versions and fact projections. |
...
| Qwen2.5-7B LoRA local runtime | Connected, measured, deployment-blocked | Local NF4 v2 runs through API/browser/SQLite with simulated-offline guards. The frozen same-local unquantized comparator does not fit this GPU, so that gate is not evaluable; false refusals and descriptive historical gaps block deployment. Mock remains the default. See `docs/PHASE_2_LOCAL_INFERENCE.md`. |
...
Observed on 2026-08-24: 50 backend/API/process/provider/vault/security tests ran
with 49 passing and one opt-in real-provider restart test skipped; frontend lint, typecheck and
production build passed. The default-mock Playwright suite passed 2/2 after an
arbitrary manual-note flow and desktop/mobile visual QA. The earlier opt-in real
local-v2 browser evidence remains recorded in `docs/PHASE_2_LOCAL_INFERENCE.md`.
