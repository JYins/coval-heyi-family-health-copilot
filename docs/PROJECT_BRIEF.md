...
**为什么这不是教程复刻**：通用 instruct 模型在"中文医疗报告结构化 + 安全拒答"这个组合上很弱。微调专打这两点，且全程被一套领域 eval harness 量化（第 7 节）。**评测才是简历皇冠**——会发 LoRA 的人一堆，认真做领域评测的少。A100 的价值不是"先扫一堆实验矩阵"而是让我们快速跑出 baseline → 产品脊柱 → 7B LoRA/QLoRA 主 adapter → 复评 → 接回产品的闭环；rank、数据量、14B、DPO/ORPO 和约束解码都作为 failure-triggered follow-up，而不是第一目标。

**简历那行字（成品长这样）**：
> Fine-tuned and evaluated an open Chinese medical-record structuring LLM on A100, producing a product-integrated LoRA/QLoRA adapter for a private longitudinal family-health assistant. Built a domain-specific eval harness for extraction F1, safety refusal, crisis-symptom recall, hallucination and overdiagnosis rates, then used failure-driven follow-up experiments such as constrained JSON decoding, data augmentation, rank/data-size ablations, and optional 14B QLoRA only when metrics justified them.
...
- **微调框架**：HuggingFace TRL `SFTTrainer` + PEFT（LoRA / QLoRA）。默认只训 adapter，方便上传 HF；第一目标是训练出能接入产品脊柱的 adapter，而不是先扫完整实验矩阵。
- **A100 使用策略**：先跑 7B baseline，再训练一个产品优先的 7B LoRA/QLoRA SFT 主 adapter（默认 rank 16，围绕 extraction + safety，summary 只在数据够稳时混入）。把 adapter 接回本地假数据产品链路，确认它真的让 OCR/ASR stub → structure → timeline → doctor summary → safety escalation 更好用。
- **可解释复杂度**：rank 8/16/32、数据量 1k/5k/20k、14B QLoRA、structured JSON constrained decoding、failure-driven data augmentation、safety refusal DPO/ORPO 都保留，但只在 baseline 或主 adapter failure 支持时启用。每个复杂组件都必须回答一个明确 product blocker 或 metric 问题。
