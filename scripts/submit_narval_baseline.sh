#!/usr/bin/env bash
#SBATCH --job-name=lora_health_baseline_qwen7b
...
mkdir -p "${PROJECT_ROOT}/results/baseline" "${PROJECT_ROOT}/slurm_logs"
cd "${CODE_DIR}"

python train/run_baseline.py \
  --gold eval/gold/synthetic_v0.jsonl \
  --out "${PROJECT_ROOT}/results/baseline/predictions.jsonl" \
...
python eval/run_eval.py \
  --gold eval/gold/synthetic_v0.jsonl \
  --pred "${PROJECT_ROOT}/results/baseline/predictions.jsonl" \
  --out "${PROJECT_ROOT}/results/baseline/metrics.json" \
  --details-out "${PROJECT_ROOT}/results/baseline/example_details.json"
