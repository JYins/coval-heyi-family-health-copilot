#!/usr/bin/env bash
#SBATCH --job-name=lora_health_sft_v2
...
CONFIG_PATH="${CODE_DIR}/configs/qwen7b_lora_sft_v2.yaml"
OUT_DIR="${PROJECT_ROOT}/runs/qwen7b_lora_sft_v2"

if [ "${LORA_HEALTH_APPROVED_SFT_V2:-0}" != "1" ]; then
  echo "Refusing to submit SFT v2: set LORA_HEALTH_APPROVED_SFT_V2=1 only after explicit user approval." >&2
  exit 4
fi
...
python scripts/validate_sft_smoke_dataset.py \
  --train-jsonl data/public/sft_v2/train.jsonl \
  --val-jsonl data/public/sft_v2/val.jsonl \
  --manifest data/public/sft_v2/manifest.json \
  --report "${PROJECT_ROOT}/results/sft_v2_validation_remote.json"
