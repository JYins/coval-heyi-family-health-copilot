#!/usr/bin/env bash
#SBATCH --job-name=lora_health_sft_v1
...
CONFIG_PATH="${CODE_DIR}/configs/qwen7b_lora_sft_v1.yaml"
OUT_DIR="${PROJECT_ROOT}/runs/qwen7b_lora_sft_v1"

if [ "${LORA_HEALTH_APPROVED_SFT_V1:-0}" != "1" ]; then
  echo "Refusing to submit SFT v1: set LORA_HEALTH_APPROVED_SFT_V1=1 only after explicit user approval." >&2
  exit 4
fi
...
python scripts/validate_sft_smoke_dataset.py \
  --train-jsonl data/public/sft_v1/train.jsonl \
  --val-jsonl data/public/sft_v1/val.jsonl \
  --manifest data/public/sft_v1/manifest.json \
  --report "${PROJECT_ROOT}/results/sft_v1_validation_remote.json"
