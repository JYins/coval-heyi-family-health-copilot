#!/usr/bin/env bash
#SBATCH --job-name=lora_health_sft_smoke_v0
...
CONFIG_PATH="${CODE_DIR}/configs/qwen7b_lora_sft_smoke.yaml"
OUT_DIR="${PROJECT_ROOT}/runs/qwen7b_lora_sft_smoke_v0"

if [ "${LORA_HEALTH_APPROVED_SFT:-0}" != "1" ]; then
  echo "Refusing to submit SFT: set LORA_HEALTH_APPROVED_SFT=1 only after explicit user approval." >&2
  exit 4
fi
...
python train/train_sft.py \
  --config "${CONFIG_PATH}" \
  --train-jsonl data/public/sft_smoke_v0/train.jsonl \
  --val-jsonl data/public/sft_smoke_v0/val.jsonl \
  --output-dir "${OUT_DIR}" \
  --local-files-only
